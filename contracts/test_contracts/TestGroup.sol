 ...
 */
contract TestGroup is IGroup, Secondary {
    ...
    modifier liquidityLock() {
        if (!isLobby()) {
            uint range = timer.getCurrent() - origin;
    ...
    function stopGroup() public onlySecretary {
        ...
        uint range = timer.getCurrent().sub(periodOrigin);
    ...
    function activePeriod() public view returns (uint) {
        ...
        if(timer.getCurrent() >= postLock && postLock != 0)
    ...
    function getSubperiod(uint _period) public view returns (uint currentSubperiod) {
        ...
        if (preLock == 0 || preLock > timer.getCurrent())
            ...
            for (uint i = 0; i < SUBPERIOD_DAYS.length.sub(1); i++) {
                if(periodLocks[_period][i.add(1)] > timer.getCurrent())
                    return (i.add(1));
            }
    ...
    function setLocks(uint _period) internal {
        if(periodLocks[_period][0] == 0) {
            uint periodOrigin = timer.getCurrent();
